<h1 align="center" style="color:#140084;">Tempo-Run</h1>
<p align="center" style="color:#4a4a4a;">Code used for Tempo Run</p>
<p align="center" style="color:#4a4a4a;">Sentiment analysis</p>
<ol style="color:#4a4a4a;">
  <li>Mở Google Colab</li>
  <li>Upload 3 file teencode.txt và icon.txt lên và data_phase_2.txt (data vòng private)</li>
  <li>Chạy từng block trong Colab, mỗi block sẽ cho ra kết quả preprocessing của data là 1 file</li>
  <li>Chạy hết thì file submit.txt cuối cùng là file nộp.</li>
</ol>
<p align="center" style="color:#4a4a4a;">Enjoy Tempo-Run!</p>
<div style="position: absolute; bottom: 0; left: 0; font-size: 6px;">
    <p style="padding-left: 10px;">Thank for</p>
    <p style="padding-left: 20px;">(https://github.com/wonrax/phobert-base-vietnamese-sentiment)</p>
    <p style="padding-left: 30px;">(https://huggingface.co/cardiffnlp/twitter-roberta-base-sentiment?text=Speaking+with+a+central+accent+is+a+bit+hard+to+hear.)</p>
</div>
